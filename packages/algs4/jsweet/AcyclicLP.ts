...
export class AcyclicLP {
  ...
  public constructor(G: EdgeWeightedDigraph, s: number) {
    ...
    for (let v = 0; v < G.V(); v++) {
      this.__distTo[v] = Number.NEGATIVE_INFINITY;
    }
    this.__distTo[s] = 0.0;
    const topological: Topological = new Topological(G);
    if (!topological.hasOrder()) throw new Error('Digraph is not acyclic.');
    for (let index121 = topological.order().iterator(); index121.hasNext(); ) {
      const v = index121.next();
      {
        for (let index122 = G.adj(v).iterator(); index122.hasNext(); ) {
          const e = index122.next();
          this.relax(e);
        }
      }
    }
  ...
  public hasPathTo(v: number): boolean {
    this.validateVertex(v);
    return this.__distTo[v] > Number.NEGATIVE_INFINITY;
  }
  ...
  public static main(args: string[]) {
    ...
    const lp: AcyclicLP = new AcyclicLP(G, s);
    for (let v = 0; v < G.V(); v++) {
      {
        if (lp.hasPathTo(v)) {
          StdOut.printf('%d to %d (%.2f)  ', s, v, lp.distTo(v));
          for (let index123 = lp.pathTo(v).iterator(); index123.hasNext(); ) {
            const e = index123.next();
            {
...
AcyclicLP.__class = 'edu.princeton.cs.algs4.AcyclicLP';
